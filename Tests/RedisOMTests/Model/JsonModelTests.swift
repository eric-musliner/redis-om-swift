...
final class JsonModelTests {
    ...
    @Test func testSaveWithUpdates() async throws {
        var user: User = User(
            name: "Alice",
            email: "alice@example.com",
            aliases: ["Alicia", "alice"],
        ...
        // Fetch JSON from Redis
        var response = try await self.poolService.leaseConnection { connection in
            connection.send(
                command: "JSON.GET",
                with: [.bulkString(ByteBuffer(string: user.redisKey))]
            )
        ...
        var jsonString = try #require(response.string)
        var data = try #require(jsonString.data(using: .utf8))
        ...
        var fetchedUser = try #require(fetched)
        fetchedUser.name = "Alicia"
        try await fetchedUser.save()

        // Fetch JSON from Redis
        response = try await self.poolService.leaseConnection { connection in
            connection.send(
                command: "JSON.GET",
                with: [.bulkString(ByteBuffer(string: user.redisKey))]
            )
        ...
        jsonString = try #require(response.string)
        data = try #require(jsonString.data(using: .utf8))

        decoded = try JSONDecoder().decode(User.self, from: data)
        #expect(decoded.id == user.id)
        #expect(decoded.name == "Alicia")
        #expect(decoded.email == "alice@example.com")
        #expect(decoded.aliases == ["Alicia", "alice"])
        #expect(decoded.age == 33)
        #expect(decoded.createdAt == user.createdAt)
    ...
    @Test func testSaveWithIdNotAutoGenerated() async throws {
    ...
    @Test func testSaveWithNestedModels() async throws {
        ...
        #expect(decoded.notes?[0].id == user.notes![0].id)
        #expect(decoded.createdAt == user.createdAt)
    }
    ...
    @Test func testGetById() async throws {
        ...
        // Get from redis
        let actual: User = try #require(try await User.get(id: user.id!))
        ...
        #expect(actual.notes?[0].id == user.notes![0].id)
